...
| PythonBridgeManager                         | plan 的 Python actions/resources、Mem0 三项配置、根据 agentPlan 创建 PythonRunnerContext | open                         | **plan 级**                        | per-plan Python runtime(interpreter/executor/runner context/adapter)+ 切换点的模块 overlay 激活:pemja MULTI_THREAD 下所有 interpreter 共享同一 CPython 的 sys.modules/sys.path,新 artifact 靠 `plan_artifact.activate_plan_artifact` 在 drain 后按"来源 + 遮蔽名"两条规则逐出旧模块并换血 sys.path(见 Proposed Changes) |
...
2.  `prepareSnapshotPreBarrier(checkpointId)`:pendingPlan 存在时,先无界 drain(复用 `waitInFlightEventsFinished`),后调 PlanVersionManager.switchToPending,再经 `PythonBridgeManager.activatePlan` 完成 Python 模块 overlay 切换(逐出旧 artifact 模块与被遮蔽的基础层模块、换血 sys.path、执行延迟的 MCP discovery)并释放旧 plan 的 Python runtime,刷新 durable 的 activePlanVersion。
...
### 新增:plan_artifact —— Python 模块世界的切换

**问题**:pemja `MULTI_THREAD` 下所有 PythonInterpreter 共享同一个 CPython,`sys.modules`/`sys.path` 进程一份。import 先查 `sys.modules` 缓存,artifact path 前置无法让已 import 过的同名模块更新;且 prepare 期改共享 path 会让还在跑的旧 plan 惰性 import 到新代码,破坏版本纯净性。

**模型**:用户代码分两层——作业提交时分发的**基础层**(始终在 sys.path)与当前动态 artifact 的**覆盖层**(前置)。切换 = 覆盖层整体替换,唯一安全点是 drain 之后(旧 plan 无任何在飞 Python 执行);`PythonBridgeManager.activatePlan` 在两个时机调用:算子 `open()`(恢复出的生效 plan 可能自带 artifact,此时无退役 plan)与 barrier 切换点。

**逐出规则**(`flink_agents/runtime/plan_artifact.py`,遍历全量 `sys.modules`):

1.  **按来源**:模块文件位于被退役 artifact 目录下的一律逐出。判据是物理位置而非 import 可达性,天然覆盖 action 的全部传递 import(含函数体内惰性 import)。

2.  **按名字**:新 artifact 提供的顶层模块名(目录扫描得出,含子树)无论当初从哪一层加载一律逐出——这是 artifact 能遮蔽已 import 的基础层模块的机制。

未被遮蔽的基础层模块不逐出、不重载,保持对象身份,既有引用继续有效。框架模块(`flink_agents`/`pemja` 等)受保护永不逐出。同一激活动作还完成:sys.path 覆盖层换血(移除退役 artifact 路径,前置 runtime path + 新 artifact 路径)、清理退役 runtime scope 的 Python function cache。

**时序配合**:prepare(`handleOperatorEvent`)照旧预建 interpreter/executor,但 artifact 路径只做校验(存在性 + sha256)不进 sys.path;会 eager import 用户代码的 MCP discovery 从 prepare 延迟到激活点。plan 反序列化与 runner context 创建只碰框架模块和惰性描述符,prepare 期不会加载用户代码。

**契约与边界**(需在用户文档写明):

*   版本纯净性在 Python 侧为 per-module 的 overlay 语义:未随本次更新的基础层模块若以 `from x import y` 早绑定持有被遮蔽模块的旧对象,切换后会新旧混用(任何进程内 reload 方案都追不回已拷贝的对象引用)。缓解:更新的最小单位是内聚的包——模块与其紧密协作者要么一起进 artifact,要么都不进。

*   overlay 非粘性:某次 artifact 不再提供某名字时,该名字回落到基础层版本。

*   进程内方案只能更新纯 Python 用户代码;site-packages/C 扩展依赖变更无法生效(C 扩展不可 reload),需重启作业(长期方向:per-plan Python worker 进程,同时可解依赖升级)。
