...
class PlanUpdateResult:
    """Confirmation that the JobManager accepted and staged an AgentPlan.

    ``registered_subtasks`` is the number of subtask gateways registered when
    the request was accepted. It does not mean the staged plan was announced or
    became effective.
    """
...
class AgentPlanClient(ABC):
    """Client for staging AgentPlan updates on a running Flink job."""
