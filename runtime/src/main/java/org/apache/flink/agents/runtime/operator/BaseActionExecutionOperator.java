...
/**
 * Base operator that executes the actions defined in the agent. This class contains the core
 * implementation logic for action execution.
 *
 * <p>Upon receiving data from the upstream, it first wraps the data into an {@link InputEvent}. It
 * then invokes the corresponding action that is interested in the {@link InputEvent}, and collects
 * the output event produced by the action.
 *
 ...
 *
 * <p>This base class is designed to be extended by version-specific implementations (e.g., {@link
 * ActionExecutionOperator}) that may need to customize behavior for different Flink versions.
 */
public class BaseActionExecutionOperator<IN, OUT> extends AbstractStreamOperator<OUT>
        implements OneInputStreamOperator<IN, OUT>, BoundedOneInput {

    private static final long serialVersionUID = 1L;

    private static final Logger LOG = LoggerFactory.getLogger(BaseActionExecutionOperator.class);
    ...
    public BaseActionExecutionOperator(
            AgentPlan agentPlan,
            Boolean inputIsJava,
